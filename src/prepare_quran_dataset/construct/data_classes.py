...
class Reciter(BaseDatasetModel):
    id: int = Field(default=-1, description="The ID of the reciter starting of 0")
    arabic_name: str = Field(description="ArabicName(الاسم)")
    english_name: str = Field(arabic_name="الاسم باللغة الإنجليزية")
...
class Moshaf(BaseDatasetModel):
    ...
    name: str = Field(description='The arabic name of the moshaf i.e "المصحف المرتل"')
    ...
    sources: list[str] = Field(description="List of urls to download recitations")
    ...
    num_recitations: int = Field(
        default=0, description="Number of recitations inside the Moshaf"
    )
    total_duraion_minutes: float = Field(
        default=0.0, description="The total duration of Moshaf in Minutes"
    )
    is_complete: bool = Field(
        default=False, description="If the Moshaf has all the recitations (114)"
    )
    ...
    publisher: Optional[str] = Field(
        default="", description="Publisher that records the recitations"
    )
    comments: Optional[str] = Field(default="", description="optional comments")
    total_size_mb: float = Field(default=0.0, description="The total size in MegaBytes")
    is_downloaded: bool = Field(
        default=False, description="moshaf is downloades using the sources or not"
    )
    alias_start_sec: float = Field(
        default=0, description="The non quranic start time in secods"
    )
