...
from prepare_quran_dataset.hf_dataset_config import HFDatasetBuilder, HFDatasetConfig, HFDatasetSplit
...
def write_redmme(
    ...
    configs.append(
        HFDatasetConfig(
            ...
            features=moshaf_features,
            data_files=[HFDatasetSplit(
                split='train',
                path=(
                        dataset_path / 'moshaf_pool.parquet').relative_to(dataset_path),
    ...
    configs.append(
        HFDatasetConfig(
            ...
            data_files=[HFDatasetSplit(
                split='train',
                path=(
                        dataset_path / 'reciter_pool.parquet').relative_to(dataset_path),
...
def write_tracks_metadata(
    ...
    for moshaf in moshaf_pool:
        ...
        if metadata:
            save_jsonl(
                metadata,
                dataset_dir / moshaf.path / 'metadata.jsonl')
